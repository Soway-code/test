...
#define	DS18B20_DQ_IN_1  HAL_GPIO_ReadPin(DS18B20_GPIOX_1,DS18B20_GPIO_PINX_1)  
...
* @brief 配置管脚为推挽输出模式
*/
void DS18B20_IO_OUT_1()
{
	GPIO_InitTypeDef GPIO_Initure;
   // RCC_GPIOX_CLK_1;									///开启GPIOA时钟
	
    GPIO_Initure.Pin=DS18B20_GPIO_PINX_1;           		///A15
    GPIO_Initure.Mode=GPIO_MODE_OUTPUT_PP;  			///推挽输出
    GPIO_Initure.Pull=GPIO_PULLUP;         				///上拉
    GPIO_Initure.Speed=GPIO_SPEED_FREQ_HIGH;     		///高速
    HAL_GPIO_Init(DS18B20_GPIOX_1,&GPIO_Initure);     	///初始化
}

/**
* @brief 配置管脚为浮空输入模式
*/
void DS18B20_IO_IN_1()
{
	GPIO_InitTypeDef GPIO_Initure;
   // RCC_GPIOX_CLK_1;									//开启GPIOA时钟
	
    GPIO_Initure.Pin=DS18B20_GPIO_PINX_1;     			//PA15
...
*/
void DS18B20_DQ_OUT_1(uint8_t out)
{
	if(1 == out)
		HAL_GPIO_WritePin(DS18B20_GPIOX_1, DS18B20_GPIO_PINX_1,GPIO_PIN_SET);		//输出高电平
	else
		HAL_GPIO_WritePin(DS18B20_GPIOX_1, DS18B20_GPIO_PINX_1,GPIO_PIN_RESET);		//输出低电平
}
...
{   
	uint8_t retry=0;
	DS18B20_IO_IN_1();    				//设置为输入
    while (DS18B20_DQ_IN_1&&retry<160)	//等待低电平脉冲到来时间 15~60us
	{
		retry++;
		delay_tim_1us();	
	};	 
    ...
    while (!DS18B20_DQ_IN_1&&retry<200)	//低电平脉冲持续时间 60~240us
	{
		retry++;
		delay_tim_1us();
	};
...
{
	uint8_t data;
	DS18B20_IO_OUT_1();   		//将DQ引脚设置为推挽输出模式
	DS18B20_DQ_OUT_1(0); 			//拉低DQ 
...
* @param[in]    dat : 要写入的字节
*/
void DS18B20_Write_Byte_1(uint8_t dat)     
{             
    uint8_t j;
    uint8_t testb;
    DS18B20_IO_OUT_1();			//设置为输出
...
{   						               
    DS18B20_Rst_1();	   			//复位ds18b20传感器
    DS18B20_Check_1();	 		//检测DS18B20的存在
...
uint8_t DS18B20_Init1(void)
{
	...
	DS18B20_Rst_1();									//复位ds18b20传感器
	return DS18B20_Check_1();							//检测DS18B20的存在
}
