...
extern int at_connect(int socket, const struct sockaddr *name, socklen_t namelen);
...
static void check_netdev_internet_up_work(struct rt_work *work, void *work_data)
{
    ...
    host = (struct hostent *) pf->netdb_ops->gethostbyname(Swyurl);
    if (host == RT_NULL)
    {
        result = -RT_ERROR;
        goto __exit;
    ...
    server_addr.sin_port = htons(Swyport);
   // server_addr.sin_addr = *((struct in_addr *)host->h_addr);		
		server_addr.sin_addr.s_addr = inet_addr(Swyurl);
    ...
    send_data[11] = RT_REVISION;
	  at_connect(sockfd, (struct sockaddr *)&server_addr, sizeof(struct sockaddr));
