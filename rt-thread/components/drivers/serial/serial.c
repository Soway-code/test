...
#undef putc
#endif
//extern char uart1_int_comin;
//extern void MODBUS_ASCII_HandlRevData(uint8_t cyRevData);
...
rt_inline int _serial_int_tx(struct rt_serial_device *serial, const rt_uint8_t *data, int length)
{
	...
	//rt_kprintf("RT_DEVICE_FLAG_INT_TX\n");
    ...
    if (serial->config.bufsz == 0)
    {
        ...
        if (rx_dma->activated != RT_TRUE)
        {
            ...
            serial->ops->dma_transmit(serial, data, length, RT_SERIAL_DMA_RX);
						rt_kprintf("_serial_dma_rx activated %d\n",result);

				}
        else result = -RT_EBUSY;
        rt_hw_interrupt_enable(level);
				rt_kprintf("_serial_dma_rx result %d\n",result);
    ...
    switch (event & 0xff)
    {
        case RT_SERIAL_EVENT_RX_IND:
        {
            ...
            while (1)
            {
                ...
                if (ch == -1) break;
                /* disable interrupt */
                level = rt_hw_interrupt_disable();  //frank 
								 
							 if (uart1_int_comin ==1 )
							 	{ 
							 	  uart1_int_comin = 0;
									
							 	  MODBUS_ASCII_HandlRevData(ch);//UART1_RXBuff[2]
						 	   // rt_kprintf("%c",ch);
							 	}
							else
								{
