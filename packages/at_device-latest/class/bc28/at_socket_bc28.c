...
extern Swy_tcp_info_t _udp_conn;
extern struct sockaddr_in server_adr;
...
struct at_device *device_ctrl;
int device_socket_ctrl;
...
static int bc28_socket_close(struct at_socket *socket)
{
    ...
    resp = at_create_resp(64, 0, rt_tick_from_millisecond(3000));//frank cl
    if (resp == RT_NULL)
    {
	...
	#elif (Swy_udp == 1)
	_udp_conn.state = AT_SOCKET_CLOSED;
#endif	
    at_delete_resp(resp);
...
static int bc28_socket_create(struct at_device *device, enum at_socket_type type, uint32_t port)
{
    ...
    resp = at_create_resp(128, 0, rt_tick_from_millisecond(300));//128 300frank cl
    if (resp == RT_NULL)
    {
        LOG_E("no memory for resp create.");
        return -RT_ENOMEM;
...
#define CONN_RETRY  3
    ...
    int device_socket = (int) socket->user_data + AT_DEVICE_BC28_MIN_SOCKET;
		rt_kprintf("connect_device_socket %d %d\n",device_socket,server_adr.sock);
		//device_socket= server_adr.sock;
    ...
    resp = at_create_resp(128, 0, rt_tick_from_millisecond(300));//frank cl
    if (resp == RT_NULL)
    {
        LOG_E("no memory for resp create.");
        return -RT_ENOMEM;
    ...
    return_socket = bc28_socket_create(device, type, 0);
		rt_kprintf("connect_create_socket %d\n",return_socket);
    ...
    for(i=0; i<CONN_RETRY; i++)
    {
        ...
        bc28_socket_event_recv(device, event, 0, RT_EVENT_FLAG_OR);
						 ...
						 if (_mq_conn.mqtt_conn != 1)
							 {                                                                                 //ff2b6a51474df06f2d45255b1cbb5dcf  e938e6daf27427554a01022333b8c5c8
								 if (at_obj_exec_cmd(device->client, resp, "AT+QMTCONN=0,\"%s\",\"%s\",\"%s\"","864162041983951","864162041983951","ff2b6a51474df06f2d45255b1cbb5dcf") < 0) 
									 {
						 ...
						 if (_mq_conn.mqtt_sub != 1)
							 {
								 if (at_obj_exec_cmd(device->client, resp, "AT+QMTSUB=0,1,\"%s\",%d","quec/864162041983951/down", 2) < 0) 
									{
                            ...
                            size_t bfsz, enum at_socket_type type)
{
    ...
    struct at_device *device = (struct at_device *) socket->device;
		device_ctrl = device;
		device_socket_ctrl = device_socket;
//    struct at_device_bc28 *bc28 = (struct at_device_bc28 *) device->user_data;
    rt_mutex_t lock = device->client->lock;
    RT_ASSERT(buff);
//		rt_mutex_t socket_send_slock = RT_NULL;
// 		socket_send_slock = rt_mutex_create("soc_send_slock", RT_IPC_FLAG_FIFO);

    resp = at_create_resp(128, 0, rt_tick_from_millisecond(300));//ori 128 frank cl
    if (resp == RT_NULL)
    {
        LOG_E("no memory for resp create bit.");
        return -RT_ENOMEM;
    }
...
//    const int port = bc28_sock_info[device_socket].port;
    ...
    while (sent_size < bfsz)
    {
       ...
       rt_kprintf("send_out_type %x\n",type);
				
#if (Swy_mqtt == 1)
      rt_enter_critical();
				if (at_obj_exec_cmd(device->client, resp, "AT+QMTPUB=%d,%d,%d,%d,\"%s\"",0,0,0,0,"quec/864162041983951/up") < 0)
					{
					rt_kprintf("at_bc28_mqfail\n");
											result = -RT_ERROR;
											goto __exit;
					}
				...
				result = (int) at_client_obj_send(device->client,buff,bfsz);
				if (result == 0)
				{
				
				rt_kprintf("send_mqtt_fail\n");
						result = -RT_ERROR;
						goto __exit;
				}
				
				rt_kprintf("send_mqtt_result %d\n",result);
				rt_exit_critical();
...
//#endif 
        ...
        {
            LOG_E("%s device socket(%d) send data failed2.", device->name, device_socket);
            result = -RT_ERROR;
            goto __exit;
        }
...
void judge_recv_event_ack(int statue)
{
 if (1 == statue)
	bc28_socket_event_send(device_ctrl, SET_EVENT(device_socket_ctrl, BC28_EVENT_SEND_OK));
else
	bc28_socket_event_send(device_ctrl, SET_EVENT(device_socket_ctrl, BC28_EVENT_SEND_FAIL));
...
#define RESOLVE_RETRY  1
    ...
    resp = at_create_resp(128, 0, rt_tick_from_millisecond(300));//128 frank cl
    if (!resp)
    {
        LOG_E("no memory for resp create.");
        return -RT_ENOMEM;
...
//    struct at_device_bc28 *bc28 = RT_NULL;
    ...
    rt_kprintf("urc_send_func_comin\n");
    if (1 == status)
    {
