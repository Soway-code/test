...
#include <at_log.h>
#define AT_DEVICE_BC28_OP_BAND  8 //frank cl

#ifdef AT_DEVICE_USING_BC28

#define BC28_WAIT_CONNECT_TIME          5000
#define BC28_THREAD_STACK_SIZE          1024 // 2048 frank cl
...
//{
//#define BC28_LINK_THREAD_STACK_SIZE     (512)  //(1024 + 512) frank cl
...
#define AT_DEFAULT_TIMEOUT             5000
    ...
    while (retry_num--)
    {
        ...
        /* check SIM card */
        for (i = 0; i < CPIN_RETRY; i++) //for 107 ver frank cl
        {
            rt_thread_mdelay(1000);
    ...
    if (result == RT_EOK)
    {
        ...
        {
           // bc28_netdev_check_link_status(device->netdev);  //frank cl
        }
...
void send_cmd_exec_reset(void)
{
	at_response_t resp = RT_NULL;
	
	rt_device_t serial = rt_device_find("uart2");
	struct at_device *device = (struct at_device *) serial;
